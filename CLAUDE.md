...
- A card claims rows by `roleIds` — `proposal_role_id` and nothing else, because one role id is one post in this tree. **Not by name** (roles 1/2 were `President`/`Vice-President` before they were `Sarpanch`/`Upa-Sarpanch`, and every rename empties whichever card spelt the old one) **and not by election type either**: a position's election type is its *constituency's*, not the post's, and the two disagree across the data — Sarpanch and Upa-Sarpanch positions sit under an MPTC constituency, role 5 Corporator under two types. Pinning a card to a type made it read "Not configured" while its own rows sat in the Other group below it. An empty `roleIds` is a post with no role configured at all.
- **The role ids are the database's own, verified against it** — Mandal Parishad 4 MPTC Member, 6 MPP, 7 Vice MPP; Zilla Parishad 3 ZPTC Member, 12 ZP Chairman, 13 Vice-Chairman; Gram Panchayat Ward Member (none), 1 Sarpanch, 2 Upa-Sarpanch; Municipality 14 Ward Councillor, 8 Chairperson, 9 Vice-Chairperson; Municipal Corporation 5 Corporator, 10 Mayor, 11 Deputy Mayor. Only the panchayat ward seat has no `proposal_role` row of its own. **Role 14 Ward Councillor was added to the database after the tree was written** — municipal-ward seats used to be filed under role 5 Corporator, and until the card was pointed at 14 all 3,352 of them fell into the Other group. Between them the fifteen cards claim every `proposal_position` in the database, so the Other group is empty — if a row ever appears there, a new `proposal_role` was added and no card claims it.
