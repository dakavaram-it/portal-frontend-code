...
and staging, same sequential assignCandidate write. getPositionsWithCandidates returns `proposal_constituency_id`, which is
...
`getCadreScores(mids)` is the one call behind the member card's score badge and its Member Since / Renewals fields (`getCadreScores`, `?mids=` comma-separated). It answers `{configured: false, candidates: []}` rather than failing when the ratings database is unset on the server, which is a state the UI renders ("No score") rather than an error. **The score half is optional; the profile half never is** — every profile field comes off the searchCadre/getProposalCandidates row the caller already had.
